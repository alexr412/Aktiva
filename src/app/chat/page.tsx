...
const EmptyState = ({ language }: { language: string }) => (
  <div className="flex flex-1 flex-col items-center justify-center gap-4 p-10 text-center h-full">
    ...
    <p className="text-slate-500 dark:text-neutral-400 font-medium max-w-xs text-sm">
    ...
    <Button asChild className="rounded-2xl h-12 px-8 font-black shadow-lg shadow-primary/20 bg-emerald-500 hover:bg-emerald-600">
...
export default function ChatPage() {
  ...
  const renderContent = () => {
    ...
    return (
      <div className="px-4 py-3 space-y-3 pb-32 max-w-5xl mx-auto w-full">
        {filteredChats.map((chat) => {
          ...
          return (
            <Link 
              ...
              href={`/chat/${chat.id}`} 
              className={cn(
                "bg-white dark:bg-neutral-900 rounded-3xl p-3.5 shadow-xl shadow-slate-200/50 dark:shadow-none transition-all duration-300 flex items-center gap-3.5 border border-transparent active:scale-[0.99] hover:border-slate-200 dark:hover:border-neutral-800",
            ...
            >
              ...
              >
                ...
                {!isDM && <div className="absolute bottom-2 right-2 w-2.5 h-2.5 rounded-full bg-emerald-500 border-[1.5px] border-white shadow-xs" />}
              </div>
