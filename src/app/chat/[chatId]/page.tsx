      ...
      <div className="flex h-full w-full bg-slate-50 dark:bg-black/95 overflow-hidden">
        ...
        <div onClick={() => setActiveMenuMessageId(null)} className="flex-1 flex flex-col h-full min-w-0 overflow-hidden relative">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 flex-1">
            <Button 
              variant="ghost" 
              size="icon" 
              className="h-10 w-10 rounded-full text-slate-500 dark:text-neutral-400 shrink-0 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors" 
              onClick={() => router.push('/chat')}
              title={language === 'de' ? 'Chat schließen' : 'Close chat'}
            >
              <X className="hidden lg:block h-5 w-5 text-slate-500 hover:text-slate-900 dark:text-neutral-400 dark:hover:text-white" />
              <ArrowLeft className="lg:hidden h-5 w-5" />
              <span className="sr-only">{language === 'de' ? 'Chat schließen' : 'Close chat'}</span>
            </Button>
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              {isDirectMessage && otherUser ? (
                  <Link href={`/users/${otherUser.uid}`} className="flex items-center gap-3 truncate hover:opacity-80 transition-opacity cursor-pointer">
                      <ProfileAvatar 
                        className="h-10 w-10 sm:h-11 sm:w-11 shadow-sm border border-white dark:border-neutral-800 shrink-0"
                      ...
                      <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-1.5 truncate">
                          <h1 className="font-black text-base sm:text-lg text-slate-900 dark:text-neutral-100 truncate">{formatFirstName(otherUser.displayName, "User")}</h1>
                          <UserBadge isPremium={otherUser.isPremium} isSupporter={otherUser.isSupporter} isCreator={otherUser.isCreator} size="sm" />
                        </div>
                        {otherUser.username && (
                          <span className="text-[11px] font-bold text-slate-400 dark:text-neutral-500 truncate">
                  ...
                  >
                    {activity ? (
                      ...
                      <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-2xl bg-primary/10 flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-105">
                        <Users className="h-5.5 w-5.5 text-primary" />
                      </div>
                    )}
                    <div className="flex flex-col min-w-0 flex-1">
                      <h1 className="font-black text-base sm:text-lg text-slate-900 dark:text-neutral-100 truncate group-hover:text-primary transition-colors flex items-center gap-1.5 leading-tight">
                      ...
                      <span className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-neutral-400 truncate block mt-0.5">
        ...
        {chat?.pinnedMessages && chat.pinnedMessages.length > 0 && (
          <div className="sticky top-16 z-10 w-full bg-white/95 dark:bg-neutral-900/95 backdrop-blur-md border-b border-slate-200 dark:border-neutral-800 px-4 py-2 flex items-center justify-between shadow-xs animate-in slide-in-from-top-2 duration-250">
            <div className="flex items-center gap-2.5 min-w-0 flex-1 max-w-4xl mx-auto">
