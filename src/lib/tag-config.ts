...
export const getPrimaryIconData = (place: any): TagStyle => {
  ...
  if (tags.includes('entertainment.zoo') || name.includes('zoo') || name.includes('tierpark')) {
    return { icon: PawPrint, color: '#755317ff', label: 'Zoo/Tierpark', bgClass: 'bg-amber-50' };
  }
  ...
  if (tags.includes('entertainment.culture.theatre') || name.includes('theater')) {
    return { icon: Theater, color: '#ec4899', label: 'Theater', bgClass: 'bg-pink-50' };
  }
  if (tags.includes('entertainment.culture.arts_centre') || name.includes('galerie') || name.includes('kunstzentrum')) {
    return { icon: Image, color: '#ec4899', label: 'Galerie', bgClass: 'bg-pink-50' };
  }
  ...
  if (name.includes('quest') || name.includes('escape') || name.includes('rätsel')) {
    return { icon: Gamepad2, color: '#f59e0b', label: 'Escape Game', bgClass: 'bg-amber-50' };
  }

  if (name.includes('wasser') || name.includes('bad') || name.includes('schwimm')) {
    return { icon: Droplets, color: '#0ea5e9', label: 'Wasserpark', bgClass: 'bg-sky-50' };
  }

  // --- PRIORITÄT 2.5: Nachtleben ---
  if (tags.includes('adult.nightclub') || name.includes('club') || name.includes('disco') || name.includes('nachtclub')) {
    return { icon: Moon, color: '#a855f7', label: 'Club', bgClass: 'bg-purple-50' };
  }
  ...
  if (name.includes('kulturzentrum') || name.includes('kultur') || name.includes('theater') || name.includes('lichtburg')) {
    return { icon: Palette, color: '#ec4899', label: 'Kultur', bgClass: 'bg-pink-50' };
  }
  ...
  if (tags.includes('pet.dog_park') || n.includes('wiese') || n.includes('park') || n.includes('gehege') || n.includes('garten')) {
    return { icon: Trees, color: '#22c55e', label: 'Natur', bgClass: 'bg-green-50 dark:bg-neutral-800' };
  }
  ...
  if (tags.some((t: string) => t.startsWith('catering.cafe') || t.startsWith('catering.bar') || t.startsWith('catering.pub')) || name.includes('cafe') || name.includes('bar') || name.includes('kneipe') || name.includes('pub')) {
    return { icon: tags.includes('catering.pub') || name.includes('pub') || name.includes('kneipe') ? BottleWine : Coffee, color: '#d97706', label: 'Bar/Pub', bgClass: 'bg-amber-50' };
  }
  if (tags.some((t: string) => t.startsWith('catering')) || name.includes('restaurant')) return { icon: Utensils, color: '#ef4444', label: 'Gastronomie', bgClass: 'bg-red-50' };
  if (tags.some((t: string) => t.startsWith('sport')) || name.includes('sport')) return { icon: Dumbbell, color: '#3b82f6', label: 'Sport', bgClass: 'bg-blue-50' };
  if (tags.includes('entertainment.cinema') || name.includes('kino')) return { icon: Film, color: '#8b5cf6', label: 'Kino', bgClass: 'bg-purple-50' };
  if (tags.includes('entertainment.amusement_arcade') || name.includes('arcade') || name.includes('spielhalle')) {
    return { icon: Gamepad2, color: '#8b5cf6', label: 'Spielhalle', bgClass: 'bg-purple-50' };
  }
