...
import { Loader2, Search, UserPlus, Check, AtSign, Smartphone, Users } from 'lucide-react';

interface AddFriendDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialTab?: 'username' | 'contacts';
}

export function AddFriendDialog({ open, onOpenChange, initialTab = 'username' }: AddFriendDialogProps) {
  const { user: currentUser, userProfile } = useAuth();
  const { toast } = useToast();
  
  const [activeTab, setActiveTab] = useState<'username' | 'contacts'>(initialTab);
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      const userProfileResult = await findUserByUsername(searchCode.trim().toLowerCase());
      if (userProfileResult) {
        setSearchedUser({ id: userProfileResult.uid, ...userProfileResult });
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent 
        className="w-full border-slate-100 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-6 sm:p-7 shadow-2xl overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] sm:max-w-md max-sm:fixed max-sm:left-1/2 max-sm:right-auto max-sm:top-[45%] max-sm:bottom-auto max-sm:w-[calc(100%-2rem)] max-sm:max-w-none max-sm:-translate-x-1/2 max-sm:-translate-y-1/2 max-h-[85vh] overflow-y-auto"
      >
        ...
        <DialogHeader className="text-left space-y-0">
          ...
          <DialogDescription className="text-sm text-slate-500 dark:text-neutral-400 font-medium leading-relaxed mt-3.5 sm:mt-4">
            Suche nach Nutzern oder finde Freunde aus deinen Kontakten.
          </DialogDescription>
        </DialogHeader>

        {/* Tab Switcher */}
        <div className="flex bg-slate-100 dark:bg-neutral-900 p-1 rounded-2xl mt-4">
          <button
            type="button"
