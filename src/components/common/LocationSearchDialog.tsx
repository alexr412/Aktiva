...
export function LocationSearchDialog({
  ...
  // Auto focus input when dialog opens on desktop or mobile
  useEffect(() => {
    if (open && isPremium) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={handleClose}>
      <DialogPrimitive.Portal>
        ...
        {/* Responsive Content Container: Bottom Sheet on Mobile (<md), Centered Dialog on Desktop (>=md) */}
        <DialogPrimitive.Content
          className={cn(
            'fixed z-50 bg-white dark:bg-neutral-950 flex flex-col outline-none shadow-2xl overflow-hidden transition-all',
            // Mobile (<md): Bottom Sheet
            'inset-x-0 bottom-0 top-auto w-full max-w-full rounded-t-[28px] rounded-b-none',
            'border-t border-slate-200/80 dark:border-neutral-800/80',
            'px-5 pt-3 pb-[calc(1.25rem+env(safe-area-inset-bottom,0px))]',
            'max-h-[70dvh]',
            'data-[state=open]:animate-in data-[state=closed]:animate-out',
            'data-[state=open]:slide-in-from-bottom-full data-[state=closed]:slide-out-to-bottom-full duration-300 ease-out',
            // Desktop (>=md): Compact Centered Dialog
            'md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:bottom-auto md:right-auto md:inset-auto',
            'md:w-full md:max-w-[540px] md:rounded-[24px]',
            'md:border md:border-slate-200/80 md:dark:border-neutral-800/80',
            'md:p-7 md:max-h-[85vh]',
            ...
            'md:data-[state=open]:slide-in-from-bottom-0 md:data-[state=closed]:slide-out-to-bottom-0 md:duration-200'
          )}
        >
          {/* Drag Handle Bar (Mobile Only) */}
          <div className="w-10 h-1 bg-slate-300 dark:bg-neutral-700 rounded-full mx-auto mb-3 shrink-0 md:hidden" />
          ...
          <div className="mt-5">
            ...
            {/* Results List */}
            <div className="mt-4 space-y-1.5 max-h-[260px] md:max-h-[300px] overflow-y-auto pr-1">
              {isLoading && (
