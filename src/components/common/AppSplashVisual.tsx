...
export interface AppSplashVisualProps {
  progress?: number;
  isFading?: boolean;
  fixed?: boolean;
  className?: string;
...
export function AppSplashVisual({
  progress,
  isFading = false,
  fixed = true,
  className,
}: AppSplashVisualProps) {
  const isIndeterminate = progress === undefined;
  const currentProgress = progress !== undefined ? Math.min(100, Math.max(0, progress)) : 0;

  return (
    <div
      className={cn(
        fixed ? "fixed inset-0 z-[99999]" : "relative w-full h-full min-h-[400px]",
        "flex flex-col items-center justify-center bg-neutral-950 text-white transition-opacity duration-500 ease-out select-none pointer-events-auto",
