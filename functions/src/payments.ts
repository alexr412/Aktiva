...
function assertBalanceInvariants(label: string, fiatCents: number, escrowCents: number): void {
  ...
  if (escrowCents < 0) {
    throw new HttpsError("internal", `[INVARIANT VIOLATION] ${label}: escrowBalance would be negative (${escrowCents} cents).`);
  }
}
 ...
 */
async function writeDLQ(db: admin.firestore.Firestore, params: {
  operationId?: string;
  userId?: string;
  activityId?: string;
  source: string;
 ...
 */
function idempotencyExpiresAt(): Date {
  const d = new Date();
 ...
 */
export const secureCompleteActivity = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Der Nutzer muss eingeloggt sein.");
  }
  ...
  if (!activityId || !operationId) {
    throw new HttpsError("invalid-argument", "Pflichtfelder (activityId, operationId) fehlen.");
  }
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (activityData.hostId !== uid) {
        throw new HttpsError("permission-denied", "Nur der Host kann diese Aktivität manuell abschließen.");
      }
    ...
    if (error instanceof HttpsError) throw error;
    throw new HttpsError("internal", "Fehler beim Abschließen der Aktivität.");
  }
});
 ...
 */
export const secureVoteToCompleteActivity = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Der Nutzer muss eingeloggt sein.");
  }
 ...
 */
export const secureCancelActivity = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Der Nutzer muss eingeloggt sein.");
  }
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      // Authorization: only host
      if (activityData.hostId !== uid) {
        throw new HttpsError("permission-denied", "Nur der Host kann diese Aktivität stornieren.");
      }
      ...
      if (activityData.status === "completed") {
        throw new HttpsError("failed-precondition", "Eine bereits abgeschlossene Aktivität kann nicht storniert werden.");
      }
      ...
      if (activityData.isPaid && activityData.price > 0) {
        ...
        const hostSnap = await transaction.get(hostRef);
        if (!hostSnap.exists) throw new HttpsError("not-found", "Host-Profil nicht gefunden.");
        const hostData = hostSnap.data() || {};
        const hostBalances = getUserBalancesInCents(hostData);

        const participantIds: string[] = activityData.participantIds || [];
        const payingParticipants = participantIds.filter((id: string) => id !== uid);
    ...
    if (error instanceof HttpsError) throw error;
    throw new HttpsError("internal", "Fehler beim Stornieren der Aktivität.");
  }
});
 ...
 */
export const secureRequestPayout = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Der Nutzer muss eingeloggt sein.");
  }
  ...
  if (!operationId) {
    throw new HttpsError("invalid-argument", "Pflichtfeld (operationId) fehlt.");
  }
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (!userSnap.exists) {
        throw new HttpsError("not-found", "Benutzerprofil nicht gefunden.");
      }
      ...
      if (userData.kycStatus !== "verified") {
        throw new HttpsError("failed-precondition", "Identitätsprüfung (KYC) ist für Auszahlungen erforderlich.");
      }
      ...
      if (amountCents < 5000) {
        throw new HttpsError("failed-precondition", "Das Auszahlungslimit liegt bei mindestens 50,00 €.");
      }

      if (balances.fiatBalanceCents < amountCents) {
        throw new HttpsError("failed-precondition", "Ungenügendes Guthaben für diese Auszahlung.");
      }
    ...
    if (error instanceof HttpsError) throw error;
    throw new HttpsError("internal", "Fehler bei der Auszahlungsanforderung.");
  }
});
 ...
 */
export const secureLeaveActivity = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Der Nutzer muss eingeloggt sein.");
  }
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (activityData.isPaid && activityData.price > 0) {
        ...
        if (hostBalances.escrowBalanceCents < netAmountCents) {
          throw new HttpsError("internal", "[INVARIANT VIOLATION] Host escrowBalance is insufficient to refund this participant.");
        }
    ...
    if (error instanceof HttpsError) throw error;
    throw new HttpsError("internal", "Fehler beim Verlassen der Aktivität.");
  }
});
 ...
 */
export const onKycRequestCreated = onDocumentCreated("kycRequests/{requestId}", async (event) => {
 ...
 */
export const onPayoutRequestUpdated = onDocumentUpdated("payoutRequests/{requestId}", async (event) => {
 ...
 */
export const onRefundUpdated = onDocumentUpdated("refunds/{refundId}", async (event) => {
  const currData = event.data?.after.data();
  const prevData = event.data?.before.data();
