 ...
 */
export const syncUserProfileUpdates = onDocumentUpdated('users/{userId}', async (event) => {
  ...
  try {
    ...
    activitiesSnap.forEach(doc => {
      ...
      if (Array.isArray(data.participantsPreview)) {
        const previewIndex = data.participantsPreview.findIndex((p: any) => p.uid === userId);
        if (previewIndex !== -1) {
