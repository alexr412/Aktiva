...
function sanitizePlaceData(raw) {
...
exports.secureVotePlace = (0, https_1.onCall)(async (request) => {
    ...
    if (!['up', 'down', 'none'].includes(type)) {
        throw new https_1.HttpsError("invalid-argument", "type must be 'up', 'down', or 'none'.");
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        const userVoteWeights = data.userVoteWeights || {};
        const previousVote = userVotes[uid];
        ...
        if (previousVote === type) {
            ...
            if (type === 'up') {
                rawUpDelta = -1;
                weightedUpDelta = -previousWeight;
            }
            else if (type === 'down') {
                rawDownDelta = -1;
                weightedDownDelta = -previousWeight;
            }
        ...
        if (type === 'up') {
            rawUpDelta += 1;
            weightedUpDelta += weight;
        }
        else if (type === 'down') {
            rawDownDelta += 1;
            weightedDownDelta += weight;
        }
...
exports.secureVoteActivity = (0, https_1.onCall)(async (request) => {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (activityData.status === 'cancelled' || activityData.status === 'completed' || activityData.status === 'blacklisted') {
            throw new https_1.HttpsError("failed-precondition", "Voting is not allowed on completed, cancelled, or blacklisted activities.");
        }
        const userVotes = activityData.userVotes || {};
        const userVoteWeights = activityData.userVoteWeights || {};
        const previousVote = userVotes[uid];
        const previousWeight = userVoteWeights[uid] ?? 1;
        let weightedUpDelta = 0, weightedDownDelta = 0;
