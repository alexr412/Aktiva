...
function mockModule(moduleName, mockExports) {
    try {
...
const isServerTimestamp = (val) => val && val.__type === "serverTimestamp";
const isIncrement = (val) => val && val.__type === "increment";
const isArrayUnion = (val) => val && val.__type === "arrayUnion";
const isArrayRemove = (val) => val && val.__type === "arrayRemove";
const isDelete = (val) => val && val.__type === "delete";
const mockFieldValue = {
    serverTimestamp: () => ({ __type: "serverTimestamp" }),
    increment: (n) => ({ __type: "increment", value: n }),
    arrayUnion: (...elements) => ({ __type: "arrayUnion", value: elements }),
    arrayRemove: (...elements) => ({ __type: "arrayRemove", value: elements }),
    ...
    update(ref, data) {
        ...
        for (const key of Object.keys(data)) {
            ...
            if (isArrayRemove(val)) {
                current[key] = current[key] || [];
                current[key] = current[key].filter((el) => !val.value.includes(el));
                continue;
            }
            if (key.includes(".")) {
                const parts = key.split(".");
...
const mockDb = {
    collection: (path) => new MockCollectionReference(path),
...
// ─── IMPORTS UNDER TEST ──────────────────────────────────────────────────────
const { getUserBalancesInCents, assertBalanceInvariants, secureJoinPaidActivity, secureRequestPayout, secureCompleteActivity, secureLeaveActivity, secureCancelActivity, } = require("./payments");
// ─── TEST SUITES ─────────────────────────────────────────────────────────────
...
async function testSecureJoinPaidActivity() {
    ...
    resetMockDb();
    await assert.rejects(secureJoinPaidActivity({ activityId: "act1", transactionToken: "pi_test_act1" }, null), (err) => err.name === "HttpsError" && err.code === "unauthenticated");
    // Test Case B: missing arguments
    await assert.rejects(secureJoinPaidActivity({ activityId: "act1" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "invalid-argument");
    // Test Case C: activity not found
    await assert.rejects(secureJoinPaidActivity({ activityId: "missing_act", transactionToken: "pi_test_missing_act" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "not-found" && err.message.includes("Aktivität nicht gefunden"));
    ...
    await assert.rejects(secureJoinPaidActivity({ activityId: "cancelled_act", transactionToken: "pi_test_cancelled_act" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("nicht mehr aktiv"));
    await assert.rejects(secureJoinPaidActivity({ activityId: "completed_act", transactionToken: "pi_test_completed_act" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("nicht mehr aktiv"));
    // Test Case E: activity not paid
    mockDbState["activities"] = {
        free_act: { status: "active", isPaid: false, hostId: "host1", participantIds: [] }
    };
    await assert.rejects(secureJoinPaidActivity({ activityId: "free_act", transactionToken: "pi_test_free_act" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("keine Zahlung"));
    ...
    await assert.rejects(secureJoinPaidActivity({ activityId: "full_act", transactionToken: "pi_test_full_act" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "resource-exhausted");
    ...
    const dupResult = await secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_test_dup_token" }, { uid: "user1" });
    ...
    const alreadyParticipantResult = await secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_test_already_participant" }, { uid: "user1" });
    ...
    const validResult = await secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_test_valid", referralId: "referrer1" }, { uid: "user1" });
    ...
    await assert.rejects(secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_test_dlq_token" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "internal" && err.message.includes("Transaktionsfehler"));
    ...
    await assert.rejects(secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_failed" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Zahlung nicht erfolgreich"));
    await assert.rejects(secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_wrong_currency" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Zahlungswährung"));
    await assert.rejects(secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_wrong_amount" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Zahlungsbetrag"));
    await assert.rejects(secureJoinPaidActivity({ activityId: "paid_act", transactionToken: "pi_wrong_metadata" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Aktivitäts-ID"));
...
async function testSecureRequestPayout() {
    ...
    // Test Case A: unauthenticated request
    resetMockDb();
    await assert.rejects(secureRequestPayout({ amount: 60.0, operationId: "op1" }, null), (err) => err.name === "HttpsError" && err.code === "unauthenticated");
    // Test Case B: missing operationId
    await assert.rejects(secureRequestPayout({ amount: 60.0 }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "invalid-argument");
    // Test Case C: user not found
    await assert.rejects(secureRequestPayout({ amount: 60.0, operationId: "op1" }, { uid: "missing_user" }), (err) => err.name === "HttpsError" && err.code === "not-found");
    // Test Case D: KYC status not verified
    mockDbState["users"] = {
        user1: { kycStatus: "pending", fiatBalance: 10000, escrowBalance: 0, balancesInCents: true }
    };
    await assert.rejects(secureRequestPayout({ amount: 60.0, operationId: "op1" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("KYC"));
    // Test Case E: amount below minimum (minimum is 50.00 EUR = 5000 cents)
    mockDbState["users"]["user1"].kycStatus = "verified";
    await assert.rejects(secureRequestPayout({ amount: 49.99, operationId: "op1" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("50,00"));
    // Test Case F: insufficient fiat balance
    await assert.rejects(secureRequestPayout({ amount: 150.0, operationId: "op1" }, { uid: "user1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Ungenügendes Guthaben"));
    // Test Case G: valid payout request
    const validResult = await secureRequestPayout({ amount: 60.0, operationId: "op_valid" }, { uid: "user1" });
    ...
    const dupResult = await secureRequestPayout({ amount: 60.0, operationId: "op_valid" }, { uid: "user1" });
    assert.deepStrictEqual(dupResult, { success: true, duplicated: true });
...
async function testSecureCompleteActivity() {
    ...
    resetMockDb();
    await assert.rejects(secureCompleteActivity({ activityId: "act1", operationId: "op1" }, null), (err) => err.name === "HttpsError" && err.code === "unauthenticated");
    // Test Case B: rejects missing required arguments
    await assert.rejects(secureCompleteActivity({ activityId: "act1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "invalid-argument");
    // Test Case C: rejects non-existent activity
    await assert.rejects(secureCompleteActivity({ activityId: "missing_act", operationId: "op1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "not-found" && err.message.includes("Aktivität nicht gefunden"));
    // Test Case D: rejects non-host caller
    mockDbState["activities"] = {
        act1: { status: "active", isPaid: true, hostId: "host1", participantIds: ["host1", "user2"] }
    };
    await assert.rejects(secureCompleteActivity({ activityId: "act1", operationId: "op1" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "permission-denied");
    ...
    const compResult = await secureCompleteActivity({ activityId: "act1", operationId: "op1" }, { uid: "host1" });
    ...
    await assert.rejects(secureCompleteActivity({ activityId: "act1", operationId: "op1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("bereits storniert"));
    ...
    const completeResult = await secureCompleteActivity({ activityId: "act1", operationId: "op_complete" }, { uid: "host1" });
    ...
    await secureCompleteActivity({ activityId: "act1", operationId: "op_cap" }, { uid: "host1" });
    ...
    await secureCompleteActivity({ activityId: "act1", operationId: "op_free" }, { uid: "host1" });
    ...
    const dupResult = await secureCompleteActivity({ activityId: "act1", operationId: "op_dup" }, { uid: "host1" });
    assert.deepStrictEqual(dupResult, { success: true, duplicated: true });
    ...
    await assert.rejects(secureCompleteActivity({ activityId: "act1", operationId: "op_fail" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "internal" && err.message.includes("Fehler beim Abschließen"));
    mockDb.runTransaction = originalRunTransaction;
    const dlqEntries = Object.values(mockDbState["failed_operations"] || {});
    assert.strictEqual(dlqEntries.length, 1);
    const dlqEntry = dlqEntries[0];
...
async function testSecureLeaveActivity() {
    ...
    resetMockDb();
    await assert.rejects(secureLeaveActivity({ activityId: "act1", operationId: "op1" }, null), (err) => err.name === "HttpsError" && err.code === "unauthenticated");
    await assert.rejects(secureLeaveActivity({ activityId: "act1" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "invalid-argument");
    ...
    await assert.rejects(secureLeaveActivity({ activityId: "comp_act", operationId: "op1" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("nicht verlassen"));
    await assert.rejects(secureLeaveActivity({ activityId: "canc_act", operationId: "op1" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("nicht verlassen"));
    // Test Case C: rejects user not in participants
    mockDbState["activities"] = {
        act1: { status: "active", hostId: "host1", participantIds: ["host1", "user2"] }
    };
    await assert.rejects(secureLeaveActivity({ activityId: "act1", operationId: "op1" }, { uid: "user3" }), (err) => err.name === "HttpsError" && err.code === "not-found" && err.message.includes("kein Teilnehmer"));
    // Test Case D: rejects host leaving own activity
    await assert.rejects(secureLeaveActivity({ activityId: "act1", operationId: "op1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("Als Host kannst du deine Aktivität nicht verlassen"));
    ...
    const leaveFreeResult = await secureLeaveActivity({ activityId: "act1", operationId: "op_leave_free" }, { uid: "user2" });
    ...
    const leavePaidResult = await secureLeaveActivity({ activityId: "act1", operationId: "op_leave_paid" }, { uid: "user2" });
    ...
    await assert.rejects(secureLeaveActivity({ activityId: "act1", operationId: "op_leave_insufficient" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "internal" && err.message.includes("insufficient to refund"));
    // Test Case H: duplicate operationId is idempotent
    mockDbState["processed_operations"] = {
        op_leave_dup: { operationType: "leave_activity" }
    };
    const leaveDupResult = await secureLeaveActivity({ activityId: "act1", operationId: "op_leave_dup" }, { uid: "user2" });
    ...
    await assert.rejects(secureLeaveActivity({ activityId: "act1", operationId: "op_leave_fail" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "internal");
    mockDb.runTransaction = originalRunTransaction;
    const dlqEntries = Object.values(mockDbState["failed_operations"] || {});
    assert.strictEqual(dlqEntries.length, 1);
    const dlqEntry = dlqEntries[0];
...
async function testSecureCancelActivity() {
    ...
    resetMockDb();
    await assert.rejects(secureCancelActivity({ activityId: "act1", operationId: "op1" }, null), (err) => err.name === "HttpsError" && err.code === "unauthenticated");
    // Test Case B: rejects missing arguments
    await assert.rejects(secureCancelActivity({ activityId: "act1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "invalid-argument");
    // Test Case C: rejects non-existent activity
    await assert.rejects(secureCancelActivity({ activityId: "missing_act", operationId: "op1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "not-found" && err.message.includes("Aktivität nicht gefunden"));
    // Test Case D: rejects non-host caller
    mockDbState["activities"] = {
        act1: { status: "active", hostId: "host1", participantIds: ["host1", "user2"] }
    };
    await assert.rejects(secureCancelActivity({ activityId: "act1", operationId: "op1" }, { uid: "user2" }), (err) => err.name === "HttpsError" && err.code === "permission-denied");
    ...
    const cancelDupStateResult = await secureCancelActivity({ activityId: "act1", operationId: "op1" }, { uid: "host1" });
    ...
    await assert.rejects(secureCancelActivity({ activityId: "act1", operationId: "op1" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "failed-precondition" && err.message.includes("bereits abgeschlossen"));
    ...
    const cancelFreeResult = await secureCancelActivity({ activityId: "act1", operationId: "op_cancel_free" }, { uid: "host1" });
    ...
    const cancelPaidResult = await secureCancelActivity({ activityId: "act1", operationId: "op_cancel_paid" }, { uid: "host1" });
    ...
    assert.ok(refunds.every((r) => r.userId !== "host1"));
    const refundUser2 = refunds.find((r) => r.userId === "user2");
    ...
    const refundUser3 = refunds.find((r) => r.userId === "user3");
    ...
    assert.ok(ledgers.every((l) => l.toUser !== "host1"));
    const ledgerUser2 = ledgers.find((l) => l.toUser === "user2");
    ...
    await secureCancelActivity({ activityId: "act1", operationId: "op_cancel_softcap" }, { uid: "host1" });
    ...
    assert.ok(softcapRefunds.every((r) => r.deficit === undefined));
    assert.ok(Object.values(mockDbState["financial_ledger"] || {}).every((l) => l.deficit === undefined));
    // Test Case K: duplicate operationId is idempotent
    mockDbState["processed_operations"] = {
        op_cancel_dup: { operationType: "cancel_activity" }
    };
    const cancelDupResult = await secureCancelActivity({ activityId: "act1", operationId: "op_cancel_dup" }, { uid: "host1" });
    ...
    await assert.rejects(secureCancelActivity({ activityId: "act1", operationId: "op_cancel_fail" }, { uid: "host1" }), (err) => err.name === "HttpsError" && err.code === "internal");
    mockDb.runTransaction = originalRunTransaction;
    const dlqEntries = Object.values(mockDbState["failed_operations"] || {});
    assert.strictEqual(dlqEntries.length, 1);
    const dlqEntry = dlqEntries[0];
