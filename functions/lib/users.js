...
exports.syncUserProfileUpdates = (0, firestore_1.onDocumentUpdated)({
    ...
    activitiesSnap.forEach(doc => {
        ...
        if (data.hostId === userId) {
            ...
            if (data.hostUsername !== newUsername) {
                updates.hostUsername = newUsername;
                needsUpdate = true;
            }
    ...
    try {
        const result = await db.runTransaction(async (transaction) => {
            ...
            transaction.update(referrerRef, {
                pendingReferrals: firestore_2.FieldValue.increment(1)
            });
 ...
 */
async function maybeActivateReferral(referredUserId, activationReason) {
    const db = admin.firestore();
    try {
        await db.runTransaction(async (transaction) => {
            ...
            transaction.update(referredRef, {
                ...
                referralActivatedAt: firestore_2.FieldValue.serverTimestamp(),
 ...
 */
async function findFriendRequestNotifRefs(db, recipientId, senderId) {
    const refs = [];
 ...
 */
exports.secureDeclineFriendRequest = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Der Nutzer muss eingeloggt sein.');
    }
    const fromUserId = request.data?.fromUserId; // The user who sent the request
    ...
    try {
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', error.message || 'Fehler beim Ablehnen der Freundschaftsanfrage.');
    }
});
/**
 * Callable function to securely cancel a sent friend request.
 */
exports.secureCancelFriendRequest = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Der Nutzer muss eingeloggt sein.');
    }
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            if (currentUserSnap.exists) {
                transaction.update(currentUserRef, {
                    friendRequestsSent: firestore_2.FieldValue.arrayRemove(toUserId)
                });
            }
            if (toUserSnap.exists) {
                transaction.update(toUserRef, {
                    friendRequestsReceived: firestore_2.FieldValue.arrayRemove(currentUserId)
                });
            }
            let unreadToDecrement = 0;
            for (const snap of notifSnaps) {
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', error.message || 'Fehler beim Zurückziehen der Freundschaftsanfrage.');
    }
});
 ...
 */
exports.matchContacts = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated.');
    }
    const callerUid = request.auth.uid;
    ...
    if (contactsPayload.length > 100) {
        throw new https_1.HttpsError('invalid-argument', 'Batch limit exceeded.');
    }
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (minuteRequestCount >= 3) {
            throw new https_1.HttpsError('resource-exhausted', 'Rate limit exceeded. Please try again in a minute.');
        }
        if (dailyIdentifierCount + uniqueEmails.length > 500) {
            throw new https_1.HttpsError('resource-exhausted', 'Daily contact matching limit reached.');
        }
        tx.set(rateLimitRef, {
            minuteWindowStart,
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'Contact lookup service error.');
    }
    const callerSnap = await db.collection('users').doc(callerUid).get();
    const callerData = callerSnap.exists ? callerSnap.data() : {};
    const callerFriends = new Set(callerData.friends || []);
    const callerSent = new Set(callerData.friendRequestsSent || []);
    const callerReceived = new Set(callerData.friendRequestsReceived || []);
    const callerBlacklist = callerData.blacklist || {};
    const callerBlockedList = new Set([
    ...
    for (const userRecord of userRecords) {
        ...
        if (targetUid === callerUid)
            continue;
        if (callerBlockedList.has(targetUid))
            continue;
